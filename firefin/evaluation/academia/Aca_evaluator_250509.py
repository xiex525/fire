...
class AcademiaEvaluator:
    ...
    def get_sorted_portfolios(self, quantiles: int = 5, plot=True) -> dict:
        """
        将股票按因子值分组，计算各组未来平均收益，并检验 Q5-Q1 收益差异的显著性（t 检验）
        输入：
            quantiles: 分组数（如 5 表示 Q1~Q5）
            plot: 是否绘图
        输出：
            dict，包括：
                - returns_by_quantile: 每组每日平均收益（DataFrame）
                - long_short: Q5 - Q1 每日收益（Series）
                - mean_diff: Q5-Q1 收益的均值
                - t_stat: Q5-Q1 收益的 t 值
                - p_value: t 检验的双尾 p 值

        工具函数接口预设:
        def compute_sorted_returns(factor: pd.DataFrame, 
                           forward_returns: ForwardReturns, 
                           quantiles: int = 5) -> pd.DataFrame:
        # 应该实现逻辑为：按日对因子分组，然后在每组上计算 forward_returns 的平均

        def plot_sorted_returns(result: pd.DataFrame):
        # 可视化函数，画出每组收益的柱状图或累计收益图
        """
        from scipy.stats import ttest_1samp  # 放在函数内导入以保持模块清洁

        cache_key = ("sorted_portfolios", quantiles)
        if cache_key not in self._result:
            quantile_ret = compute_sorted_returns(self.factor, self.forward_returns, quantiles)

            # 提取 Q1 和 Q5（默认 Q1 最小因子组，Q5 最大因子组）
            q1 = quantile_ret.iloc[:, 0]
            q5 = quantile_ret.iloc[:, -1]
            long_short = q5 - q1

            # 计算均值、t 值和 p 值
            mean_diff = long_short.mean()
            std_diff = long_short.std()
            n = long_short.count()
            t_stat = mean_diff / (std_diff / (n ** 0.5)) if n > 1 and std_diff > 0 else float("nan")
            _, p_value = ttest_1samp(long_short.dropna(), popmean=0)

            self._result[cache_key] = {
                "returns_by_quantile": quantile_ret,
        ...
        if plot:
            plots.plt_quantile_cumulative_returns(result["returns_by_quantile"])
            plots.plt_quantile_cumulated_end_returns(result["returns_by_quantile"])
        return result
        
    def get_ic_ir(self, method: typing.Literal["pearson", "spearman", "kendall"] = "spearman", plot=True) -> dict:
        """
        计算因子的横截面信息系数（IC）、信息比率（IR）和t检验统计量
        输入：
            method: 使用的相关系数计算方法
            plot: 是否绘图
        输出：
            dict，包括：
                - ic_series: 每期 IC（Series）
                - mean_ic: 平均 IC
                - std_ic: IC 标准差
                - ir: 信息比率（= mean / std）
                - t_stat: t 统计量（用来检验 mean_ic 是否显著大于 0）

        工具函数接口预设:
        def compute_cross_section_ic_ir(factor: pd.DataFrame, 
                                 forward_returns: ForwardReturns, 
                                 method: str = "spearman") -> dict:
        def plot_ic_series(ic_series: pd.Series):
        # 绘制 IC 的时间序列图
        def plot_ic_histogram(ic_series: pd.Series):
        # 绘制 IC 的分布直方图
        """
        cache_key = ("ic_ir", method)
        if cache_key not in self._result:
            result = compute_cross_section_ic_ir(self.factor, self.forward_returns, method)
            ...
            n = ic_series.count()
            t_stat = mean_ic / (std_ic / (n ** 0.5)) if n > 1 and std_ic > 0 else float("nan")
        ...
        result = self._result[cache_key]
        if plot:
            plots.plot_ic_series(result["ic_series"])
            plots.plot_ic_histogram(result["ic_series"])
        return result

    def get_cross_sectional_regression(self, mode: typing.Literal["single", "fama_macbeth"] = "fama_macbeth", 
                                        date: str = None, plot=True) -> dict:
        """
        在每一期进行横截面回归（OLS），支持：
            - 单期回归（single）：仅计算指定某一期的结果
            - 多期回归（fama_macbeth）：对每一期进行横截面回归，并估计系数均值和 t 值
    
        输入：
            mode: "single" 或 "fama_macbeth"
            date: 单期模式下指定日期（如 '2022-01-01'）
            plot: 是否绘图
    
        输出：
            dict，包括：
                - mode: 回归模式
                - coefs: 系数结果（单期为 Series，多期为 DataFrame）
                - mean_coef: 各因子平均系数（多期模式下才有）
                - t_stat: 各因子 t 值（多期模式下才有）
                - p_value: 各因子 p 值（多期模式下才有）
    
        工具函数接口预设:
        def run_single_cross_section_regression(factor: pd.DataFrame, 
                                                forward_returns: ForwardReturns, 
                                                date: str) -> pd.Series:
            # 返回该日期上所有因子的回归系数（Series）
    
        def run_fama_macbeth_regression(factor: pd.DataFrame, 
                                         forward_returns: ForwardReturns) -> dict:
            # 返回字典，包括：
            # {
            #   "coefs": DataFrame(日期 × 因子),
            #   "mean_coef": Series,
            #   "t_stat": Series,
            #   "p_value": Series
            # }
        def plot_coef_time_series(coefs: pd.DataFrame):
        # 输入为时间 × 因子的回归系数表，画出各因子的时间序列变化
        """
        cache_key = ("cross_section_regression", mode, date)
        if cache_key not in self._result:
            if mode == "single":
                if date is None:
                    raise ValueError("单期回归模式下必须指定 date 参数")
                coefs = run_single_cross_section_regression(self.factor, self.forward_returns, date)
                result = {
                    "mode": "single",
                    "coefs": coefs
                }
            elif mode == "fama_macbeth":
                result = run_fama_macbeth_regression(self.factor, self.forward_returns)
                result["mode"] = "fama_macbeth"
            else:
                raise ValueError("mode 仅支持 'single' 或 'fama_macbeth'")
    
            self._result[cache_key] = result
    
        result = self._result[cache_key]
        # 可选绘图（例如绘制各因子回归系数时间序列）
        if plot and result["mode"] == "fama_macbeth":
            plots.plot_coef_time_series(result["coefs"])
        return result
